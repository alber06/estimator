...
Each subproject runs its own `docker compose` stack (Compose v2.15+). Both join a shared Docker network named `estimator_shared` so Rails can reach the FastAPI container by name at `http://estimator:8000`.

```bash
# 1. Start the API first (from estimator-backend/)
docker compose up --build

# 2. Then start Rails (from estimator-web/)
cp .env.example .env   # optional local overrides
docker compose up --build
```

When launched from the **monorepo root** (`docker compose up` in `../`, Compose v2.20+), all services share one network automatically — same `http://estimator:8000` URL applies.
