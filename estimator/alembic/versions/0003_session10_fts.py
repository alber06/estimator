...
Text-search configuration: the budget corpus is in Spanish (seed budgets under
``data/seed/budgets/``, phase names like "Diseño UX" / "Pruebas y despliegue"),
so we use the ``spanish`` regconfig for stemming and stop-words. The same
config must be used in ``plainto_tsquery`` (repository) or the GIN index is
bypassed and matching silently changes.
